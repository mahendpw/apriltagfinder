...
void AprilTagFinder::setTagCodes(string s)
{
    if (s == "16h5")
    {
...
void AprilTagFinder::parseOptions(int argc, char* argv[])
{
    int c;
...
void AprilTagFinder::setupVideo()
{
...
void AprilTagFinder::print_detection(AprilTags::TagDetection& detection) const
{
    ...
    cout << "  distance=" << translation.norm() << "m, x=" << translation(0)
            << ", y=" << translation(1) << ", z=" << translation(2) << ", yaw="
            << yaw << ", pitch=" << pitch << ", roll=" << roll << endl;
...
void AprilTagFinder::processImageGray(cv::Mat& image_gray)
{
    printf("%s %d going to process the image with apriltags \n", __FUNCTION__, __LINE__);
    ...
    printf("%s %d finished processing the image with apriltags \n", __FUNCTION__, __LINE__);
...
void AprilTagFinder::processImage(cv::Mat& image, cv::Mat& image_gray)
{
    ...
    this->processImageGray(image_gray);
...
void AprilTagFinder::loadImages()
{
    cv::Mat image;
    cv::Mat image_gray;

    for (list<string>::iterator it = m_imgNames.begin(); it != m_imgNames.end();
            it++)
    {
